...
Have something in your code that you want to reverse but can't remember the Git command? Just follow the below flowcharts. Generated diagrams using [mermaid](https://mermaid-js.github.io/mermaid/).

```mermaid
flowchart TD
    A[I want to view code in previous commits] -- ...in the same branch --> B[git checkout &lt;COMMIT-HASH&gt;]
...
```mermaid
flowchart TD
    A[I want to undo uncommitted changes in the current index] -- ...for all files --> B[git reset --hard]
    A -- ...for a specific file --> C[git checkout -- &lt;FILE-PATH&gt;]
```

```mermaid
flowchart TD
    A[I want to remove files from the staging area] -- ...for all files --> B[git reset]
    A -- ...for a specific file --> C[git reset &lt;FILE-PATH&gt;]
```

```mermaid
flowchart TD
    A[I want to undo the effects of one or more commits] -- ...by removing the commits from history --> B[keep changes in the current index?]
    ...
    A -- ...by creating new commits --> G[undo a specific commit or several commits?]
    G -- ...undo a specific commit --> H[git revert &lt;COMMIT-HASH&gt;]
    G -- ...several commits at once -->  I[from the HEAD or between specific commits?]
